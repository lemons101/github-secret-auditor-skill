...
description: 当用户希望让 OpenClaw 优先通过 ACP 调度 Claude Code，对 GitHub 仓库进行 API Key、Token、密码、私钥、Webhook URL 等敏感信息泄露巡检、自动修复、验收、推送修复 commit，并通过飞书发送巡检报告时使用。
---
...
自动化任务也应优先使用 ACP 调度 Claude Code。不要因为后台任务不能直接发送聊天 slash command，就跳过 ACP 或改用 OpenClaw 自己手工巡检。

如果后台通道不能发送 `/acp spawn`、`/acp steer` 这类 slash command，OpenClaw 应调用同一 ACP 后端能力完成 spawn/steer；只有 ACP 后端能力确实不可用时，才允许降级到 Claude Code headless CLI。

如果当前环境既没有可用 ACP 调度能力，也无法调用 Claude Code CLI，应返回 `failed` 并说明阻塞原因。
...
- ACP 后端健康，`/acp doctor` 或等价 ACP 后端健康检查应显示 `healthy: yes`。
- `acpx` 后端已启用，`registeredBackend` 应为 `acpx`。
- ACPX 已在课程环境初始化阶段配置为允许 Claude Code 对授权仓库执行必要写入。
- 降级通道中，`claude -p "只回复 OK"` 应能正常返回。
- 目标仓库已获得用户授权。
- 默认工作根目录为 `/srv/openclaw-runner`。

如果 ACP 后端不可用，且降级 CLI 也不可用，应先停止任务并提示用户修复 Claude Code/ACP 配置，不要进入代码巡检。

### Claude Code 调度通道

本 Skill 支持两种调度通道，但默认顺序必须是 ACP first：

- 默认通道：OpenClaw ACP 调度。交互场景可用 `/acp spawn claude` 和 `/acp steer --session ...`；后台任务应调用 OpenClaw 的 ACP 后端能力完成同样的 spawn/steer，而不是把 slash command 当 shell 命令执行。
- 降级通道：Claude Code headless CLI，例如 `claude -p "..."`。仅当 ACP 后端不可用、或当前部署尚未暴露后台 ACP 调度接口时使用。

前置部署参考：需要配置 OpenClaw、Claude Code、ACP 或 ACPX 写入权限时，读取 `references/preflight_setup.md`。执行巡检任务时不要默认加载该文件。

Skill 只做检查和提示：

- 如果聊天 slash command 不可用，应尝试 OpenClaw ACP 后端能力，不要直接判定 ACP 不可用。
- 如果 ACP 后端能力不可用，才尝试 Claude Code CLI。
- 如果 ACP 和 CLI 都不可用，应返回 `failed`，不要手工替代 Claude Code。
...
- 通过 ACP 创建 Claude Code 会话。交互场景可用 `/acp spawn claude`；后台场景应调用等价 ACP 后端能力。
- 通过 ACP steer 将巡检任务投递给 Claude Code。
- 仅当 ACP 后端不可用时，才通过 Claude Code headless CLI 降级投递巡检任务。
...
5. 优先使用 ACP 创建 Claude Code 会话并记录 `session-key`。
6. 使用 ACP steer 将巡检任务投递给 Claude Code。
7. 如果 ACP 后端能力不可用，才降级使用 Claude Code headless CLI 投递任务，并在报告中说明降级原因。
...
OpenClaw 不应自己手工替代 Claude Code 做代码巡检和修复。自动化任务和飞书交互演示都应优先使用 ACP，把 Claude Code 启动成外部 Coding Agent 会话。
...
### 3. 默认：使用 ACP 调度 Claude Code

交互场景可以直接使用 slash command：

```text
/acp spawn claude --mode persistent --thread off --cwd /srv/openclaw-runner/repos/agentic-ai
```

后台任务不能假设可以发送聊天 slash command。此时应调用 OpenClaw 暴露的 ACP 后端能力，完成与上面 slash command 等价的操作：

```text
acp.spawn(agent="claude", mode="persistent", thread="off", cwd="/srv/openclaw-runner/repos/agentic-ai")
```

OpenClaw 必须记录返回的完整 `session-key`，例如 `agent:claude:acp:...`。

### 3.1 投递任务给 Claude Code
...
```text
acp.steer(session="<session-key>", prompt="<读取任务包并执行巡检修复的 prompt>")
```

同一 `session-key` 可以多轮 steer。OpenClaw 可以先投递巡检修复任务，验收后如果发现 `.env.example`、README、`.gitignore` 或报告摘要不完整，再用同一 session 追加一轮定向修正。
...
### 4. 降级：Claude Code CLI

只有在 ACP 后端能力不可用时，才允许降级到 Claude Code CLI：
...
CLI 成功执行后，OpenClaw 应读取 stdout、Git Diff 和仓库状态进行验收。此时 `session-key` 记为 `n/a`，`runner` 记为 `claude-cli`，并在最终报告中说明 ACP 降级原因。
...
如果后台不能发送 slash command，这不是跳过 ACP 的理由；应使用 OpenClaw 的 ACP 后端能力完成同样的 spawn/steer。只有该能力未接入或返回不可用，才允许降级 CLI 或返回 `failed`。
...
- 先确认是否只是当前聊天通道不能发送 slash command；如果是，应切换到 OpenClaw ACP 后端能力。
- 执行 `/acp doctor` 或等价 ACP 后端健康检查。
- 如果 ACP 后端确实不可用，再测试 `claude -p "只回复 OK"` 作为降级。
- 如果 CLI 可用，可以使用 CLI 通道，但必须在报告中说明 `runner=claude-cli` 和 ACP 失败原因。
