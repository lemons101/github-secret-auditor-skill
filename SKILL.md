...
description: 当用户希望让 OpenClaw 通过 ACP 调度 Claude Code，对 GitHub 仓库进行 API Key、Token、密码、私钥、Webhook URL 等敏感信息泄露巡检、自动修复、生成安全报告、验收并推送修复 commit 时使用。
---
...
本 Skill 用于让 OpenClaw 调度 Claude Code，对授权 GitHub 仓库执行夜间密钥泄露巡检、安全修复、报告生成，并在验收通过后推送修复 commit 到 GitHub。

OpenClaw 负责调度、任务包、边界、验收和 GitHub 推送；Claude Code 负责进入仓库执行搜索、修改、验证和报告生成。
...
11. 返回 OpenClaw 总结：`status`、`changed_files`、`risk_summary`、`residual_risks`、`commit`、`next_action`。
...
```text
/acp steer --session <session-key> 读取 /srv/openclaw-runner/tasks/agentic-ai-secret-audit.json，并在当前仓库执行 GitHub 密钥泄露巡检与安全修复。只在当前仓库内工作，不读取 .env、私钥、SSH Key、Cookie、生产配置和用户个人目录。发现硬编码敏感值后，改为环境变量读取，补充或更新 .env.example、README、.gitignore，生成 security-report.md。最后输出修改文件清单、风险摘要、测试/静态检查结果和 git diff 摘要。不要在 Claude Code 会话内 push，不要创建 PR，不要输出完整密钥。如果无法写文件或无法生成报告，必须明确返回权限错误类型，不要只做分析。
```
...
- 必须生成或更新仓库根目录下的 `security-report.md`。
- 输出 Git Diff 摘要。
...
如果 `security-report.md` 缺失，即使代码已经修复，也不能判定为 `passed`。
...
cd /srv/openclaw-runner/repos/agentic-ai
git status --short
git add .gitignore README.md openclaw-infra/configs/.env.example openclaw.json security-report.md
...
```text
生成任务包 -> /acp spawn claude -> /acp steer 投递任务 -> 验收报告和 Diff -> push 修复 commit -> 飞书推送巡检报告
```

本 Skill 默认不创建 PR、不合并分支、不清理历史，只推送普通修复 commit。PR、历史清理和审批流可放到企业治理增强版。
...
6. 必须生成仓库根目录下的 security-report.md。
