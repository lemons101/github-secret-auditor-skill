...
- ACPX 已允许本次课程示例仓库的写入任务。课程 MVP 可使用 `permissionMode=approve-all`，否则 Claude Code 可能只能读不能写。
- 目标仓库已获得用户授权。
...
ACP 会话是非交互式运行的，无法在写文件或执行 shell 时弹出人工确认。如果 ACPX 仍使用默认读权限，Claude Code 可以完成巡检分析，但在 `Edit`、`Write` 或生成报告时会失败。

课程演示环境中，在只授权 `/srv/openclaw-runner/repos/agentic-ai` 的前提下，可以临时开启 ACPX 写入批准：
...
配置后必须重启 OpenClaw gateway，再重新执行：

```text
/acp doctor
```

确认 ACP 仍为 `healthy: yes` 后，重新 `/acp spawn claude` 创建新会话。旧会话可能仍沿用旧权限。

安全边界：

- `approve-all` 只用于课程示例仓库或明确授权仓库。
- 不要把 `--cwd` 指向 `/root`、用户 home、系统目录或包含生产密钥的目录。
- 任务 prompt 仍必须禁止读取 `.env`、私钥、Cookie、生产配置和用户个人目录。
- 任务完成后，如果不再需要自动写入，可恢复更保守的权限策略。
...
- 检查 ACPX 是否配置了 `plugins.entries.acpx.config.permissionMode=approve-all`。
- 修改 ACPX 权限后，必须重启 gateway，并重新创建 Claude ACP 会话。
