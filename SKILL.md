...
12. OpenClaw 再次验收 Git Diff 和残余风险。
13. 验收通过后，OpenClaw 执行 `git add`、`git commit`、`git push`。
14. OpenClaw 生成飞书报告；报告不得提交到 GitHub 仓库。
15. 返回最终状态、commit、风险摘要、后续动作和飞书通知结果。

第一轮 `<完整任务 prompt>` 应包含：任务包路径、`templates/acp_steer_prompt.md` 的任务要求、目标仓库路径、禁止读取范围、输出格式要求。不要只发送一句“去巡检”，也不要把修复限定成固定文件模板。
