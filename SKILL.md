...
- 每个目标仓库同一时刻只允许 1 个活动 Claude ACP child session。OpenClaw 必须先检查并发锁或任务状态，只有 `active_session_count == 0` 时才允许 `sessions_spawn`；任务结束、失败或超时后必须释放/关闭 session 并把 active 计数归零。
...
## ACP Session 生命周期与并发控制

OpenClaw 必须把 Claude ACP child session 当作需要显式管理的运行资源，而不是一次性消息。后台巡检、Heartbeat 和定时任务必须遵守以下状态机：

```text
idle(active_session_count=0)
-> acquire single-flight lock
-> sessions_spawn(mode="run") and save childSessionKey
-> running Claude Code audit/fix/verify
-> OpenClaw acceptance and optional sessions_send follow-up
-> commit/push/report
-> close or cancel child session
-> release lock and set active_session_count=0
```

硬性要求：

- 并发闸门：锁粒度至少包含 `repo_url`、`branch` 和本 Skill 名称；同一锁下如果已有活动 session，新的定时触发或重复触发必须跳过本轮，不得再创建新的 `sessions_spawn`。
- 复用规则：同一次巡检内的补漏必须使用已保存的 `childSessionKey` 和 `sessions_send`；不要为了补漏重新 `sessions_spawn`。
- 终态等待：`sessions_spawn` 返回 `accepted` 只代表已创建 child session，不代表任务结束。OpenClaw 必须等待 Claude Code 输出完成并确认任务进入 `completed`、`failed`、`cancelled` 或 `timeout` 等终态后再进入最终报告。
- TTL：每个巡检 session 必须设置最大运行时间。超过 TTL、Claude Code 无响应、验收无法继续或 push/report 阶段失败时，OpenClaw 都必须执行强制回收。
- 回收动作：成功、失败、异常、超时和用户取消都必须进入 `finally` 类清理路径，调用 OpenClaw Sessions/ACP runtime 提供的关闭、取消或回收能力释放 child session。不要只删除 `childSessionKey` 或只把任务标记完成。
- 状态记录：OpenClaw 必须记录 `session_key`、`state`、`started_at`、`deadline_at`、`closed_at`、`repo_path` 和 `lock_key`。只有完成回收并释放锁后，才允许下一次巡检进入 `idle`。
- 禁止后台 persistent：后台自动化巡检不得使用 `/acp spawn --mode persistent`。`persistent` 只允许作为人工连通性演示，并且演示结束后必须关闭该会话。
...
调用 `sessions_spawn` 前必须已经获得本 Skill 的 single-flight lock，并确认同一 `repo_url` + `branch` 没有活动 child session。否则本轮巡检应返回 `skipped_active_session` 或继续等待已有任务结果，不得创建第二个 Claude ACP 进程。
...
如果需要多轮补漏，使用同一个 `childSessionKey`：
...
注意：该 `persistent` 示例只用于人工验证飞书交互链路，不属于后台巡检流程。不要把它用于定时巡检或自动修复任务；演示完成后必须关闭/释放该 ACP 会话。
...
重新 `sessions_spawn` 前必须先确认旧 session 已关闭、取消、过期并被回收，且对应 single-flight lock 已释放。不得因为 `sessions_send` 失败就直接并发创建新的 child session。
...
2. 获取 single-flight lock；如果同一 `repo_url` + `branch` 已有活动 session，跳过本轮或等待已有任务，不得重复 `sessions_spawn`。
3. 准备工作目录：`/srv/openclaw-runner/repos`、`/srv/openclaw-runner/tasks`、`/srv/openclaw-runner/reports`。
4. 如果本地没有仓库，clone 到 `repo_path`。
5. 如果本地已有仓库，先执行 `git status --short`；如有未提交修改，停止并报告 `failed`，避免覆盖用户工作。
6. 工作区干净时执行 `git pull --ff-only`。
7. 基于 `templates/openclaw_task.secret_audit.json` 生成任务包到 `/srv/openclaw-runner/tasks/<repo>-secret-audit.json`。
8. 使用 `sessions_spawn(runtime="acp", agentId="claude", mode="run", thread=false, cwd=<repo_path>, prompt=<完整任务 prompt>)` 创建并启动 Claude Code child session。
9. 记录返回的 `childSessionKey`、`started_at`、`deadline_at` 和 `lock_key`，格式为 `agent:claude:acp:...`。
10. 等待 Claude Code 输出修改文件清单、风险摘要、测试/静态检查结果和 Git Diff 摘要；如果超过 TTL 或 session 无响应，取消/关闭 session 并报告 `failed`。
11. OpenClaw 保存 Claude 输出、`git status --short`、`git diff` 和验收结果。
12. 如验收缺失项，使用 `sessions_send(sessionKey=<childSessionKey>, prompt=<显式上下文 follow-up prompt>)` 定向补漏。follow-up prompt 必须包含上一轮输出、当前 Git Diff、验收缺失项和本轮目标。
13. OpenClaw 再次验收 Git Diff 和残余风险，并额外检查当前分支是否出现 Claude Code 提前生成的本地 commit。
14. 如果 Claude Code 已提前执行本地 `git commit`，OpenClaw 不立即 push；先核实该 commit 的修改范围、diff、是否包含禁止文件、是否满足验收标准。只有核实通过后，才允许继续 push；如未通过，则要求 Claude Code 回到未提交状态或重新修复。
15. 验收通过后，OpenClaw 执行 `git add`、`git commit`、`git push`。如本地已存在且通过验收的修复 commit，OpenClaw 可以直接复用该 commit 执行 push，并在报告中注明“Claude 已先本地 commit，OpenClaw 已验收后推送”。
16. OpenClaw 生成飞书报告；报告不得提交到 GitHub 仓库。
17. 无论状态为 `passed` 还是 `failed`，OpenClaw 都必须关闭/回收 child session，记录 `closed_at`，释放 single-flight lock，并把活动 session 计数归零。
18. 返回最终状态、commit、风险摘要、后续动作和飞书通知结果。
...
- 不要改用 OpenClaw 手工巡检。
- 必须释放 single-flight lock，并确认没有残留已创建但未记录的 child session。
...
- 如 session 丢失或过期，先取消/回收旧 session 并释放旧锁；只有重新获得 single-flight lock 后，才允许重新执行 `sessions_spawn` 并在 prompt 中带上 OpenClaw 保存的上下文。
